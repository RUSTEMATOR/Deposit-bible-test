        ...
        stage('Test Run') {
            steps {
                script {
                    ...
                    def command = """
                        sudo -E /bin/bash -c "/usr/local/bin/python3 -m pip install pytest && /usr/local/bin/python3 -m pytest -s -k test_deposit_bible -m \${params.marker} --url \${params.url} --path \${params.path} && echo 'Tests completed successfully'"
                    """
